...
def analyzer(wordsa, attemptNum):
    words = wordsa.copy()
    ...
    #If 0 words it would cause an error
        ...
        if len(words) >= 2:
            ...
            if attemptNum != 1 and attemptNum != 2 and attemptNum != 3:
                theWord 
            else:
                ...
                if(len(words)) == 0:
                    theWord = max(wordsAsProbabilitiess) #Max probability
                    theWord = wordd[wordsAsProbabilitiess.index(theWord)] 
